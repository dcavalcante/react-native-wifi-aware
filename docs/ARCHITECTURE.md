...
Normalization is only accepted where semantics match. Apple pairing/declaration constraints and Android discovery sessions are exposed as platform-aware capability differences. In particular, Apple's services are build-time `Info.plist` declarations, while Android currently accepts a runtime `serviceName`: Stage 6 must expose a documented declared-service restriction or registry on Apple and reject undeclared values rather than silently rewriting them. The Android-only example string `com.wifiaware.stage4` is not a portable library service identity. No Android↔Apple promise exists.
