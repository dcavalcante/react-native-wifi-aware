...
Status: `PROVISIONAL — COMPILE AND PHYSICAL VALIDATION PENDING`.
...
| Validation debt | **Physical verification required:** entitlement recognition, TLS/system pairing behavior, listener acceptance, client readiness, loss, close, and Xcode SDK compatibility require a paid team and two eligible iOS/iPadOS 26 devices. The user also deferred Codegen/package/Xcode build work. |
...
5. Run only static lint/type checks. The user explicitly deferred Codegen,
   package/Xcode compilation, signing, and physical execution to a later gate.

## Gate record

| Gate | State | Evidence |
|---|---|---|
| Readiness review | READY — PROVISIONAL PHYSICAL VALIDATION PENDING | Focused review initially blocked on the example sending Android-only hello after Apple listener registration; plan amended to make message/hello Android-only and expose the Apple pairing → listener → client flow. Final review then caught and verified the corrected Objective-C++ data-path forwarding/event routing. Peer categories, passphrase semantics, accepted-path ownership, lifecycle/event routing, and deferred validation boundary are coherent. |
